// server.js — VS Tools Render Engine V2 Final + MP4 Export
...
function injectRenderEngineV2(html = "") {
  const engineCSS = `
<style id="vs-render-engine-v2-css">
:root {
  --vs-bg: #05060f;
  --vs-card: rgba(255,255,255,.08);
  --vs-border: rgba(255,255,255,.16);
  --vs-glow: rgba(120,80,255,.55);
  --vs-text: #ffffff;
  --vs-muted: rgba(255,255,255,.78);
}
...
[class*="topbar"] {
  ...
  background: rgba(5, 6, 15, 0.76) !important;
...
body::before {
  ...
  animation: vsLightSweep 1.4s ease .2s both;
  z-index: 2;
}
...
@keyframes vsTextFade {
  from { opacity: 0; transform: translateY(8px); }
  to { opacity: 1; transform: translateY(0); }
}

@keyframes vsLightSweep {
...
@keyframes vsFloat {
  ...
  50% {
    transform: translate3d(30px,-50px,0) scale(1.6);
    opacity: .75;
  }
}
...
`;
  ...
  const engineJS = `
<script id="vs-render-engine-v2-js">
(function () {
  function ready(fn) {
  ...
  ready(function () {
    ...
    var items = document.querySelectorAll(
      "section, .card, .tool-card, .panel, .box, .result, .preview, [class*='card'], [class*='tool'], [class*='panel']"
    );
...
${output}
  ...
  if (output.includes("</head>")) {
    output = output.replace("</head>", `${engineCSS}</head>`);
  } else {
    output = engineCSS + output;
  }

  if (output.includes("</body>")) {
    output = output.replace("</body>", `${engineJS}</body>`);
  } else {
    output += engineJS;
  }
...
app.post("/api/render/html", (req, res) => {
  try {
    ...
    const rendered = injectRenderEngineV2(html);
...
app.post("/api/download/html", (req, res) => {
  try {
    const { html, filename = "vs-tools-render.html" } = req.body;
    ...
    const safeName = safeFilename(filename, "vs-tools-render.html");
    const rendered = injectRenderEngineV2(html);
...
app.post("/api/download/pdf", async (req, res) => {
  ...
  try {
    ...
    const safeName = safeFilename(filename, "vs-tools-render.pdf");
    const rendered = injectRenderEngineV2(html);
...
app.post("/api/download/png", async (req, res) => {
  ...
  try {
    ...
    const safeName = safeFilename(filename, "vs-tools-render.png");
    const rendered = injectRenderEngineV2(html);
...
app.post("/api/download/mp4", async (req, res) => {
  ...
  try {
    ...
    const safeName = safeFilename(filename, "vs-tools-render.mp4");
    const rendered = injectRenderEngineV2(html);
    ...
    outputPath = path.join(tempDir, `${Date.now()}-${safeName}`);
...
app.get("*", (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.send(`
      <h1>VS Tools Render Engine V2 Final</h1>
...
app.listen(PORT, () => {
  console.log(`✅ VS Tools Render Engine V2 Final running on port ${PORT}`);
});